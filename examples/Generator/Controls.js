...
export const pointerGroup = [
  ...
  },
  {
    ...
    label: 'Stroke color',
    type: 'color',
    condition: props => (props.pointerRadius > 0 && props.pointerStrokeWidth > 0)
  },
  {
...
const getControlInput = (control, props) => {
  switch(control.type) {
    case 'number':
      return (
        <input type={control.type}
               ...
               min={control.hasOwnProperty('min') ? getMinMaxValue(control.min, props) : null}
               max={control.hasOwnProperty('max') ? getMinMaxValue(control.max, props) : null}
        />
      )

    case 'color':
      return (
        <ColorSelector value={props[control.name]} 
                       onChange={getChangeFn(control, props)} 
        />
      )
    ...
    case 'linecap':
      return (
        <LinecapSelector value={props[control.name]} 
                         onChange={getChangeFn(control, props)} 
        />
      )
  }
...
const getControl = (control, props) => {
  if(control.hasOwnProperty('condition') && !control.condition(props)) return null

  const isModified = props.defaultState[control.name] !== props[control.name]

  return (
    <label key={control.name}>
      <span className={isModified ? 'caption modified' : 'caption'}>{control.label}</span>
      {getControlInput(control, props)}
      {control.hasOwnProperty('unit') && <span className="unit">{control.unit}</span>}
    </label>
  )
}
...
const renderTitle = (props) => {
  ...
  if(props.type === 'progress') return <div className="title">Progress style</div>
  if(props.type === 'track') return <div className="title">Track style</div>
  if(props.type === 'pointer') return <div className="title">Pointer style</div>
}
