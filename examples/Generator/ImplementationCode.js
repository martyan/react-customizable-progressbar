...
class ImplementationCode extends Component {
  ...
  renderButton = () => {
    ...
    return (
      <div className="btns">
        <button onClick={() => toggleCode(!codeVisible)}>{!codeVisible ? 'Show code' : 'Show progress bar'}</button>
      </div>
    )
  }
  ...
  renderGroup = (group, condition) => {
    let code = ``
    group.filter((control, i) => condition ? condition(i) : true).forEach(control => {
      if(this.isModified(control.name)) {
        switch(control.type) {
          case 'text':
          case 'linecap':
          case 'color':
            code += `    ${control.name}="${this.props[control.name]}"\n`
            break
          case 'checkbox':
            code += `    ${control.name}\n`
            break
  ...
  renderCode = () => {
    ...
    return <PrismCode component="pre" className="language-markup">{code}</PrismCode>
  }
