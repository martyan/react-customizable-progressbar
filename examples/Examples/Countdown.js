...
const Indicator = (props) => {
  ...
  return (
    <div className="indicator-countdown">
      <div>
        <div className={seconds > 0 ? 'caption' : 'caption big'}>Popcorn ready <span>in</span></div>
        <div className={seconds > 0 ? 'time' : 'time hidden'}>{getText(date)}</div>
      </div>
    </div>
  )
}
...
class Countdown extends Component {
  ...
  handleTimer = (elapsedSeconds) => {
    const progress = this.roundProgress(((elapsedSeconds + initialSeconds) / totalSeconds) * 100)
    this.setState({
      progress,
      elapsedSeconds
    })
  ...
  render = () => {
    ...
    return (
      <div className="item">
        <div className="title">
          <span>Countdown</span>
          <a href="https://github.com/martyan/react-customizable-progressbar/blob/master/examples/Examples/Countdown.js" target="_blank">
